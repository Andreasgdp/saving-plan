...
var wishItems = sqliteTable("wish_items", {
  id: text("id").primaryKey(),
...
function getDbClient() {
  ...
  if (url.startsWith("file:")) {
    try {
      const dataDir2 = path.resolve(process.cwd(), "data");
  ...
  const rawClient2 = createClient2({
    url,
    authToken
  });
  const db2 = drizzle(rawClient2, { schema: schema_exports });
  return { db: db2, rawClient: rawClient2 };
}
var { db, rawClient } = getDbClient();
...
var CURRENCY_PRESETS = {
  ...
  GBP: { code: "GBP", symbol: "\xA3", position: "prefix", decimals: 0 },
  JPY: { code: "JPY", symbol: "\xA5", position: "prefix", decimals: 0 },
...
var PERSONAL_WANTS_CONFIG = {
  name: "Personal Wants & Tech",
  currentAmountSaved: 700,
  amountToSave: 300,
  frequency: "monthly",
  savingsDayOfMonth: 25,
  firstSavingDate: (/* @__PURE__ */ new Date()).toISOString().split("T")[0],
  emergencyBuffer: 200,
  annualInterestRate: 0
};
var PERSONAL_WANTS_ITEMS = [
  {
    id: "wish-p1",
...
// src/server/planService.ts
async function getUserStoreData(userId) {
  const userRows = await db.select().from(users).where(eq(users.id, userId));
  let userSettings = { ...DEFAULT_GLOBAL_SETTINGS };
  ...
  if (userPlans.length === 0) {
    ...
    return {
      version: 3,
      activePlanId: formattedPlans2[0].id,
      settings: userSettings,
      plans: formattedPlans2,
      lastSaved: (/* @__PURE__ */ new Date()).toISOString()
    };
  }
  const formattedPlans = [];
  for (const p of userPlans) {
    const items = await db.select().from(wishItems).where(eq(wishItems.planId, p.id)).orderBy(asc(wishItems.priority));
    formattedPlans.push({
      id: p.id,
      name: p.name,
...
async function saveUserStoreData(userId, data) {
  const now = (/* @__PURE__ */ new Date()).toISOString();
  await db.insert(users).values({
    id: userId,
...
var dataDir = path2.resolve(process.cwd(), "data");
var dataFile = path2.resolve(dataDir, "plan.json");
function safeReadGuestFile() {
  try {
    if (fs2.existsSync(dataFile)) {
      return fs2.readFileSync(dataFile, "utf-8");
    }
  } catch {
  }
  return null;
}
function safeWriteGuestFile(content) {
  try {
    if (!fs2.existsSync(dataDir)) {
      fs2.mkdirSync(dataDir, { recursive: true });
    }
    fs2.writeFileSync(dataFile, content, "utf-8");
    return true;
  } catch (err) {
  ...
  try {
    ...
    if (req.method === "POST") {
      ...
      if (userId) {
        ...
        res.status(200).json({ success: true, savedAt: (/* @__PURE__ */ new Date()).toISOString(), storage: "db" });
        return;
      }
